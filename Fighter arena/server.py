...
class MultiplayerRoom:
    ...
    def snapshot_for(self, viewer_id: Optional[str] = None):
        ...
        with self._lock:
            ...
            room = self._room_info_locked(include_code=bool(viewer_id))
            return viewer, players, zombies, walls, room
    ...
    def do_GET(self):  # noqa: N802
        ...
        if parsed.path == "/api/multiplayer/state":
            ...
            if room is None:
                ...
                return
            viewer, players, zombies, walls, room_info = room.snapshot_for(viewer_id=player_id)
            self._write_json(
                200,
                {
                    "self": viewer,
    ...
    def do_POST(self):  # noqa: N802
        ...
        if parsed.path == "/api/multiplayer/update":
            ...
            if not updated_player:
                self._write_json(404, {"error": "Player not found"})
                return
            self._write_json(200, {"ok": True, "self": updated_player, "room": room.summary(include_code=True)})
            return
